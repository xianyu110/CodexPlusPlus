...
const routes: Array<{ id: Route; label: string; icon: LucideIcon }> = [
  ...
  { id: "recommendations", label: "国内中转站", icon: ExternalLink },
...
export function App() {
  ...
  const refreshAds = async (silent = false) => {
    ...
    if (result) {
      setAds(result);
      if (!silent) showResultNotice("国内中转站", result, { silentSuccess: true });
    }
  };
...
function RecommendationsScreen({ ads, actions }: { ads: AdsResult | null; actions: Actions }) {
  ...
  return (
    <>
      <Panel>
        <CardHead title="国内中转站" detail="Codex 国内登录和使用入口" />
        <CardContent>
          <div className="recommend-hero">
            <div>
              <strong>{ads ? `已加载 ${items.length} 个入口` : "尚未加载国内中转站入口"}</strong>
              <span>入口来自项目内置配置，不再拉取远程广告列表。</span>
            </div>
      ...
      <Panel>
        <CardHead title="Codex 国内使用" detail={`${items.length} 个入口`} />
        <CardContent>
          <AdGrid actions={actions} ads={items} empty="暂无国内中转站入口。" />
        </CardContent>
      </Panel>
    </>
  );
      ...
      <Panel>
        ...
        <CardContent>
          <div className="metric-list">
            ...
            <Metric label="项目地址" value="github.com/xianyu110" />
          </div>
          <Toolbar>
            <Button onClick={() => void actions.openExternalUrl("https://github.com/xianyu110/")} variant="secondary">
              <ExternalLink className="h-4 w-4" />
              打开项目主页
            </Button>
          </Toolbar>
